...
function ajaxFun(url, method, formData, dataType, fn, file=false) {
	const settings = {
		...
		beforeSend: function(jqXHR) { // 에러일때
		...
		error: function(jqXHR) {
			if(jqXHR.status === 403) { // 필터에서 로그인이 안됐으면 403 던지게 만들었다
				login();
				return false;
			...
			console.log(jqXHR.responseText); // 이게 없으면 에러 잡기 힘들어서 배포 전에 확인
		}
	};
	
	// AJAX 로 파일 보낼때 절대로 파일을 건들면 안된다
	if(file) {
	...
	$.ajax(url, settings); // 주소와 setting 분리
}
...
$(function() {
	$('.btnSend').click(function() {
		...
		let formData = {content: content}; // 쿼리 객체로 할때는 인코딩하면 안됨
